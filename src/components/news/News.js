...
function News() {
  ...
  const displayWorks = news
    .slice(pagesVisited, pagesVisited + worksPage)
    .map((item, index) => {
      return (
        <div key={index} className="Events-photo">
  ...
  return (
    <>
      <div className="main-box-events">
        <Navbar />
        <div className="organization-title">{t("news")}</div>
        <div className="boxes-organization">
          <div className="box-for-org">
            <div className="row">{displayWorks}</div>
          </div>
          <ReactPaginate
            previousLabel={
              <img src="./images/work-pagination-prev.png" alt="" />
            }
